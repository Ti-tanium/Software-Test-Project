...
with torch.no_grad():
    preds = give_test_data_in_chunks(input_data)

print(classification_report(target_data, preds))
TN, FP, FN, TP = confusion_matrix(target_data, preds, labels=[0, 1]).ravel()
...
df.to_csv('./results/flaky_test_predict_results.csv')
